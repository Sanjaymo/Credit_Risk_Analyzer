  ...
  try {
    ...
    if (error.response) {
      return res.status(error.response.status).json({
        error: error.response.data?.error || 'ML service returned an error while processing the prediction.'
      });
    }

    if (['ECONNREFUSED', 'ENOTFOUND', 'ECONNABORTED'].includes(error.code)) {
      return res.status(503).json({
        error: 'ML service is unavailable. Start the Flask service on port 8000 or set ML_SERVICE_URL correctly.'
      });
    }
